...
*/
#define CVV8_TypeName_DECL(X) template <> struct TypeName< cvv8::sl::At<0,CVV8_TYPELIST(X) >::Type > { static const char * Value; };
...
typedef boost::shared_ptr<std::string> boost_string;
template <>
struct NativeToJS<boost_string>
{
    v8::Handle<v8::Value> operator()( boost_string const & v ) const
    {
        if (NULL != v.get()) {
            return NativeToJS<std::string>()(*v);
        } else {
            return v8::Null();
        }
    }
...
/** Convenience instance of NativeToJS. */
static const NativeToJS<boost_string> BoostStdStringToJS = NativeToJS<boost_string>();
...
/** Specialization to convert JS values to std::string. */
template <>
struct JSToNative<boost_string>
{
    typedef boost_string ResultType;
    ResultType operator()( v8::Handle<v8::Value> const & h ) const
    {
        static const std::string emptyString;
        v8::String::Utf8Value const utf8String( h );
        const char* s = *utf8String;
        if (NULL != s) {
            return boost_string(new std::string(s, utf8String.length()));
        } else {
            return boost_string();
        }
    }
